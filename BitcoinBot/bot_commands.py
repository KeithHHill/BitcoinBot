...
def list_transactions(user,channel,command) :
    ...
    db = database.Database()
    records = db.fetchAll("""select *, "purchase" as type from purchases where user_id = %s and record_complete = 1""",[user])
    db.close()
...
def provide_profit_info(user,channel,command):
    ...
    for wallet in wallets:
        if wallet["coin_type"] == "btc":
            btc_worth = float(wallet["purchased"]) * current_btc_price
        elif wallet["coin_type"] == "eth":
            eth_worth = float(wallet["purchased"]) * current_eth_price
        elif wallet["coin_type"] == "ltc":
            ltc_worth = float(wallet["purchased"]) * current_ltc_price
        #sum total spent
    ...
    # response = "Your change in value: $" + str(round(total_value-float(total_spent),2))
...
def add_amount_to_purchase(user,channel,command,purchase_id) :
    ...
    if parsed_amount == 0 : # cound't get the number
